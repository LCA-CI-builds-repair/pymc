...
UTests for new `RandomVariables` are mostly located in `tests/distributions/test_*.py`.
...
3. Shape variable inference is correct, via `check_rv_size`l not be aware of all this complexity and should instead make use of helper methods such as `~pymc.CustomDist`.
